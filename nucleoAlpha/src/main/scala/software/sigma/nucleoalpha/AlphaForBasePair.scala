...
/**
  * alpha algorithm for DNA string
  * since there are only 4 types of events in traces: A, T, G, C
  * lists of direct followers and causalities overlap: A>T, T>A, G>C, C>G
  * there are no parallelism and no exclusiveness
  *
  * @param eventLog
  */
class AlphaForBasePair(eventLog: List[String]) {
  ...
  private val exclusiveness = List.empty
  footprint.buildRelations(causality, parallels, exclusiveness, directFollowers)
